...
sub get_genome {
    my $genome_ref  = shift;
    my $orig_genome = get_ws_client()->get_objects( [ { ref => $genome_ref } ] )->[ 0 ]{ data };
...
##  Features
    for my $ftr ( @{ $orig_genome->{ features } } ) {
        ...
        if ( $ftr->{ type } eq "gene" && not defined $ftr->{ protein_translation } ) {
...
##  Noncoding
    for my $ftr ( @{ $orig_genome->{ non_coding_features } } ) {

        if ( $ftr->{ functions } && @{ $ftr->{ functions } } ) {
            $ftr->{ function } = join "; ", @{ $ftr->{ functions } };
        }
        my $func = defined( $ftr->{ function } ) ? $ftr->{ function } : "";
        $orig_funcs->{ $ftr->{ id } } = $func;
...
sub set_params {
    ...
    for my $key ( keys %default_params ) {
...
sub submit_set_annotation {
    ...
    for my $obj ( keys %$genome_objs ) {
        my $name = $ws_client->get_object_info( [ { ref => $obj } ], 0 )->[ 0 ][ 1 ];
        # say "REF: $obj\tNAME: $name";
    }
    return ( $ref );
}
