...
sub get_object {
    my ( $ws, $id ) = @_;
    my $output = Bio::KBase::kbaseenv::ws_client()
        ->get_objects( [ Bio::KBase::kbaseenv::configure_ws_id( $ws, $id ) ] );
    return $output->[ 0 ]->{ data };
}

sub get_objects {
    my ( $args, $options ) = @_;
    my $input  = { objects => $args, };
...
sub add_object_created {
    my ( $parameters ) = @_;
    $parameters
        = Bio::KBase::utilities::args( $parameters, [ "ref", "description" ], {} );
    ...
    while ( $retryCount > 0 ) {
        eval {
            ...
            for ( my $i = 0; $i < @{ $output }; $i++ ) {
                ...
                if (   defined( $output->[ $i ]->[ 10 ] )
                    && defined( $output->[ $i ]->[ 10 ]->{ description } ) )
                {
